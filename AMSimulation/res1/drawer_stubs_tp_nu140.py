...
def drawer_draw(histos, options):
    def modify_barrel(h1):
        xlow, xup = -100, 100
        nbinsx = h1.GetNbinsX()
        title = "; Barrel module z position (a.u.); <# stubs/module/BX>"
        h = TH1F(h1.GetName()+"_i", title, nbinsx, xlow, xup)
        for ix in xrange(nbinsx):
            h.SetBinContent(ix+1, h1.GetBinContent(ix+1))
        return h

    def modify_endcap(h1):
        xlow, xup = 0, 15
        nbinsx = h1.GetNbinsX()-e_offset
        title = "; Endcap module #rho position (a.u.); <# stubs/module/BX>"
        h = TH1F(h1.GetName()+"_i", title, nbinsx, xlow, xup)
        for ix in xrange(nbinsx):
    ...
    def modify_tower(h1):
        xlow, xup = 0.5, 6.5
        nbinsx = h1.GetNbinsX()
        title = "; Trigger tower #eta position (a.u.); <# stubs/tower/BX>"
        h = TH1F(h1.GetName()+"_i", title, nbinsx, xlow, xup)
        for ix in xrange(nbinsx):
            h.SetBinContent(ix+1, h1.GetBinContent(ix+1))
        return h
    ...
    for i in xrange(5,11):
        ...
        h = histos[hname_px] = modify_barrel(h1)
    ...
    for i in xrange(5,11):
        tlegend.AddEntry(histos["b%i_px" % i] , writeme[i-5], "p")
    tlegend.Draw()
    CMS_label()
    save(options.outdir, "barrel", dot_root=True)
    ...
    for i in xrange(11,23):
        ...
        hname = "e%i" % i
        hname_py = "e%i_py" % i
        h = histos[hname_py] = modify_endcap(h1)
        ...
        if i-11 == 4:
            ...
            for i in xrange(11,16):
                tlegend.AddEntry(histos["e%i_py" % i] , writeme[i-11], "p")
            tlegend.Draw()
            CMS_label()
            save(options.outdir, "endcap_plus", dot_root=True)
        elif i-18 == 4:
            writeme = [
                "z = -135 cm",
                "z = -160 cm",
            ...
            for i in xrange(18,23):
                tlegend.AddEntry(histos["e%i_py" % i] , writeme[i-18], "p")
            tlegend.Draw()
            CMS_label()
            save(options.outdir, "endcap_minus", dot_root=True)
    ...
    h1 = h2.ProjectionX(hname_px)
    h1.Scale(1.0/h2.GetNbinsY())  # averaged
    h = histos[hname_px] = modify_tower(h1)

    h.SetMinimum(0); h.SetMaximum(ymax)
    h.SetStats(0); h.Draw("p")
    CMS_label()
    save(options.outdir, "tower", dot_root=True)
