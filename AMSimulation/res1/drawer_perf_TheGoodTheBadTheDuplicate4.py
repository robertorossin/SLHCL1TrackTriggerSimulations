...
col4  = TColor.GetColor("#6a3d9a")  # tttt140
fcol4 = TColor.GetColor("#cab2d6")  # tttt140
col5  = TColor.GetColor("#006400") 
fcol5 = TColor.GetColor("#8FBC8F") 
...
def ncommonSS(road1,road2):
    ...
    assert(road1[3].size()==road2[3].size())
    commomSS=0

#     for ilayer, SS in enumerate(road1[3]):
#         if ilayer in road1[2]: print bcolors.OKGREEN,road1[3][ilayer],bcolors.ENDC,
#         else: print road1[3][ilayer], 
#     print ""
#     for ilayer, SS in enumerate(road2[3]):
#         if ilayer in road2[2]: print bcolors.OKGREEN,road2[3][ilayer],bcolors.ENDC,
#         else: print road2[3][ilayer], 
#     print ""

    for ilayer, SS in enumerate(road1[3]):
#         if (SS == road2[3][ilayer] and ilayer not in road1[2] and ilayer not in road2[2]): commomSS = commomSS+1 # count common SS
        if (SS == road2[3][ilayer]): commomSS = commomSS+1 # count common SS
#     print commomSS
    return commomSS
...
parsN_1min  = [[0. ,0. ]          ,[-0.0055,-0.0048] ,[-0.011,-0.006],[-0.0025, -0.0022],[-0.516,-0.289] ]
parsN_1max  = [[50.,50.]          ,[ 0.0052, 0.0045] ,[ 0.012, 0.007],[ 0.0018,  0.0016],[ 0.514, 0.234] ]
...
#         print histos_SSclean[hnameSS].GetName()
    for hname, h in histos_SSclean.iteritems():
        histos[hname]=histos_SSclean[hname]
    del histos_SSclean

    for hname, h in histos.iteritems():
        print "Booked histogram:", hname 
        
    logics = ["_5oo6","_6oo6"]
    objects = ["tracks","roads","trkparts"]
    for iobj in range(len(objects)):
        for ipar in range(len(parnames)):
            if (iobj==0 or ipar>0):
                for logic in logics:
                    if (iobj>1):
                        if (logic!=logics[0]): continue
                        logic=""
    ...
    for hname, h in histos.iteritems():
        for c in ["ntracks", "ngoods", "nduplicates", "nfakes"]:
            if (hname.find(c)==0 and hname.find("SSclean")==-1):
...
#         histos[hname] = TH1F(hname, "; track #eta", nbins, xmin, xmax)
    ...
    nmax=options.nskip+options.nentries
    if (options.nentries<0): nmax = options.nskip+maxPlotEvts
    for ievt in range(options.nskip,nmax):
        nbins, xmin, xmax = 80, 0., 80.
        hname = "byevt_sortedRoads0_ncomb_%03d"                                                   % (ievt)
        histos[hname] = TH1F(hname, ";sorted road. evt=%d; # combs/road"                          % (ievt), nbins, xmin, xmax)
        hname = "byevt_sortedRoads1_ntracks_%03d"                                                 % (ievt)
        histos[hname] = TH1F(hname, ";sorted road. evt=%d; # tracks/road"                         % (ievt), nbins, xmin, xmax)
        hname = "byevt_sortedRoads2_ntracks_chi2_%03d"                                            % (ievt)
        histos[hname] = TH1F(hname, ";sorted road. evt=%d; # tracks redChi2<3/road"               % (ievt), nbins, xmin, xmax)
        hname = "byevt_sortedRoads3_ntracks_chi2_matched_%03d"                                    % (ievt)
        histos[hname] = TH1F(hname, ";sorted road. evt=%d; # matched tracks redChi2<3/road"       % (ievt), nbins, xmin, xmax)
        hname = "byevt_sortedRoads4_ntracks_chi2_matched_SSclean_%03d"                            % (ievt)
        histos[hname] = TH1F(hname, ";sorted road. evt=%d; # matched tracks redChi2<3/road"       % (ievt), nbins, xmin, xmax)
        hname = "byevt_sortedRoads5_firstBornRoads_%03d"                                          % (ievt)
        histos[hname] = TH1F(hname, ";first born road. evt=%d; "                                  % (ievt), nbins*10, xmin, xmax)
        nbins, xmin, xmax = 3, 0., 3.
        hname = "byevt_nroads0_%03d"                                                              % (ievt)
        histos[hname] = TH1F(hname, ";;# roads. evt=%d"                                          % (ievt), nbins, xmin, xmax)
        histos[hname].GetXaxis().SetBinLabel(1,"any")
        histos[hname].GetXaxis().SetBinLabel(2,"5/6")
        histos[hname].GetXaxis().SetBinLabel(3,"6/6")
        hname = "byevt_nroads1_SSclean_%03d"                                                      % (ievt)
        histos[hname] = TH1F(hname, ";;# clean roads evt=%d"                                  % (ievt), nbins, xmin, xmax)
        histos[hname].GetXaxis().SetBinLabel(1,"any")
        histos[hname].GetXaxis().SetBinLabel(2,"5/6")
        histos[hname].GetXaxis().SetBinLabel(3,"6/6")
        hname = "byevt_nroads2_SSclean_matched_chi2_%03d"                                         % (ievt)
        histos[hname] = TH1F(hname, ";;# clean-matched roads #Chi2<3 evt=%d"                % (ievt), nbins, xmin, xmax)
        histos[hname].GetXaxis().SetBinLabel(1,"any")
        histos[hname].GetXaxis().SetBinLabel(2,"5/6")
        histos[hname].GetXaxis().SetBinLabel(3,"6/6")
...
#         if h.ClassName() != "TEfficiency": continue
        ...
        if (hname.find("byevt")>=0): 
            if (hname.find("sortedRoads0_ncomb")>=0 or hname.find("byevt_nroads_")>=0):
                h.SetMarkerColor(col1); h.SetLineColor(col1); h.SetFillColor(fcol1)
            if (hname.find("byevt_sortedRoads1_ntracks_")>=0 or hname.find("byevt_nroads_SSclean_")>=0):
                h.SetMarkerColor(col2); h.SetLineColor(col2); h.SetFillColor(fcol2)
            if (hname.find("byevt_sortedRoads2_ntracks_chi2_")>=0):
                h.SetMarkerColor(col3); h.SetLineColor(col3); h.SetFillColor(fcol3)
            if (hname.find("byevt_sortedRoads3_ntracks_chi2_matched_")>=0):
                h.SetMarkerColor(col4); h.SetLineColor(col4); h.SetFillColor(fcol4)
            if (hname.find("byevt_sortedRoads4_ntracks_chi2_matched_SSclean_")>=0 or hname.find("byevt_nroads_SSclean_matched_chi2_")>=0):
                h.SetMarkerColor(col5); h.SetLineColor(col5); h.SetFillColor(fcol5)
            if (hname.find("byevt_sortedRoads5")>=0):
                h.SetMarkerColor(1); h.SetLineColor(1); h.SetFillColor(1)
...
def drawer_project(tree, treepattern, treepatternA, histos, options):
    ...
    for ievt, evt in enumerate(tree):
        if (ievt < options.nskip): continue
        if (ievt+options.nskip == options.nentries):  break
...
#                 continue
        ...
        for iroad, (patternRef, patternInvPt, superstripIdsA, stubRefsA) in enumerate(
            ...
            for itrack in xrange(ntracks_all):
                ...
                if (track_roadRef==iroad): road_trackList.append(itrack)
            ...
            roadList.append((patternRef,6-len(misslayers),misslayers,superstripIdsA,stubRefsA, abs(treepatternA.invPt_mean),treepatternA.phi_mean,ncombs_per_road,road_trackList))

######### By Logic and by Pt
#         roadListSorted = sorted(roadList      , key = operator.itemgetter(5)              )
#         roadListSorted = sorted(roadListSorted, key = operator.itemgetter(1), reverse=True)
######### By Logic and by popularity
#        roadListSorted = sorted(roadList      , key = operator.itemgetter(0)              )
#        roadListSorted = sorted(roadListSorted, key = operator.itemgetter(1), reverse=True)
######### By Logic and by missing layer
...
#         maxsiblings = 0
        ...
        nroads5_SSclean, nroads6_SSclean, ncombs_SSclean = 0, 0, 0
        for iroad, road in enumerate(roadListSorted):
            ...
            if nCommonLayers < options.ncommonSS :
                roadListClean.append(road)
                hname = "byevt_sortedRoads5_firstBornRoads_%03d" % (ievt)
                histos[hname].Fill(iroad+0.4,0.7)
                    
        for iroad, roadClean in enumerate(roadListClean):
            if   (roadClean[1]==6): nroads6_SSclean +=1
            elif (roadClean[1]==5): nroads5_SSclean +=1
            else                  :
                print "IT SHOULD NOT HAPPEN!!!"
                return
            stubRefsA= roadClean[4]
            ncombs_per_road = 1
            for v in stubRefsA:
                n = v.size()
                if n != 0:
                    ncombs_per_road *= n
            ncombs_SSclean += ncombs_per_road
...
#         tracks = {}
        ...
        for itrack in xrange(ntracks_all):
            ...
            isSSclean = 0
            for iclean in range(len(roadListClean)):
                if (track_pattRef == roadListClean[iclean][0]): 
                    isSSclean = 1
                    break
...
#             tracks[track_pattRef] = (track_invpt,track_eta,track_phi0,track_z0,track_roadRef,chi2,ndof)
        ...
        for itrack, track in enumerate(trackListSorted):
            ...
            chi2            = track[7]
            isSSclean       = track[8]
            if isSSclean: ntracks_SSclean += 1
            track_partmatch = -2
            ...
            for itrkpart in range(len(trkparts)):
                [pt, eta, phi, z0, charge, pdgId,trackmatches,trackmatchesSSclean]=trkparts[itrkpart]
                if                         chi2/ndof             < parsN_1max[0][i] \
                    ...
                    trkparts[itrkpart][7].append(itrack)
            ...
            if track_partCleanmatch==-2 and isSSclean: nfakes_SSclean += 1
...
#             track_invpt, track_eta, track_phi0, track_z0,track_roadRef,chi2,ndof = vtrack
        ...
        for itrkpart in range(len(trkparts)):
            ...
            for imatch in range(len(trackmatchesSSclean)):
                if (not trigger_SSclean): 
                    ngoods_SSclean += 1
                    trigger_SSclean = True
                else: nduplicates_SSclean += 1
            prefix = "efficiency_"
            if options.etamin < eta < options.etamax and options.phimin < phi < options.phimax:
                ...
                if options.ptmin < pt < options.ptmax:
                    denomEntries += 1
            ...
            if options.etamin < eta < options.etamax and options.phimin < phi < options.phimax:
                histos[prefix + "pt"      ].Fill(trigger_SSclean, pt)
                histos[prefix + "ppt"     ].Fill(trigger_SSclean, pt)
                histos[prefix + "pppt"    ].Fill(trigger_SSclean, pt)
                if options.ptmin < pt < options.ptmax:
                    if trigger_SSclean:  numEntries_SSclean += 1
            if options.ptmin < pt < options.ptmax and options.phimin < phi < options.phimax:
                histos[prefix + "eta"     ].Fill(trigger_SSclean, eta)
            if options.ptmin < pt < options.ptmax and options.etamin < eta < options.etamax:
                histos[prefix + "phi"     ].Fill(trigger_SSclean, phi)
            if options.ptmin < pt < options.ptmax and options.etamin < eta < options.etamax and options.phimin < phi < options.phimax:
                histos[prefix + "vz"      ].Fill(trigger_SSclean, vz)


        plotScales=[1.0,0.98,0.96,0.94,0.92]
        nmax=options.nskip+options.nentries
        if (options.nentries<0): nmax = options.nskip+maxPlotEvts
        if ievt in range(options.nskip,nmax):
            nroads5_SSclean_matched_chi2 = 0
            nroads6_SSclean_matched_chi2 = 0
            for iroad, road in enumerate(roadListSorted):
                isCleanRoad = False
                for iroadClean, roadClean in enumerate(roadListClean):
                    if road[0] == roadClean[0]:
                        isCleanRoad = True
                        break
                ncomb  =    road[7]
                ntracks=len(road[8])
                ntracks_chi2         = 0
                ntracks_chi2_matched = 0
                ntracks_chi2_matched_SSclean = 0
                for itrack in road[8]:
                    if itrack in tracksMatched:
                        ndof,track_pattRef,track_invpt,track_eta,track_phi0,track_z0,chi2,track_partmatch,track_partCleanmatch = tracksMatched[itrack]
                        if (chi2/ndof<options.maxChi2):
                            ntracks_chi2 += 1
                            if (track_partmatch==0 or track_partmatch==-1000000): 
                                ntracks_chi2_matched += 1
                                if isCleanRoad: ntracks_chi2_matched_SSclean += 1
                if ntracks_chi2_matched_SSclean>0:
                    if road[1]==5: nroads5_SSclean_matched_chi2 +=1
                    if road[1]==6: nroads6_SSclean_matched_chi2 +=1
                
                histos["byevt_sortedRoads0_ncomb_%03d"                       % (ievt)].Fill(iroad,ncomb                       *plotScales[0])
                histos["byevt_sortedRoads1_ntracks_%03d"                     % (ievt)].Fill(iroad,ntracks                     *plotScales[1])
                histos["byevt_sortedRoads2_ntracks_chi2_%03d"                % (ievt)].Fill(iroad,ntracks_chi2                *plotScales[2])
                histos["byevt_sortedRoads3_ntracks_chi2_matched_%03d"        % (ievt)].Fill(iroad,ntracks_chi2_matched        *plotScales[3])
                histos["byevt_sortedRoads4_ntracks_chi2_matched_SSclean_%03d"% (ievt)].Fill(iroad,ntracks_chi2_matched_SSclean*plotScales[4])
            histos["byevt_nroads0_%03d"                       % (ievt)].Fill(0,(nroads5                     +nroads6                     )*plotScales[0])
            histos["byevt_nroads0_%03d"                       % (ievt)].Fill(1,(nroads5                                                  )*plotScales[0])
            histos["byevt_nroads0_%03d"                       % (ievt)].Fill(2,(                             nroads6                     )*plotScales[0])
            histos["byevt_nroads1_SSclean_%03d"               % (ievt)].Fill(0,(nroads5_SSclean             +nroads6_SSclean             )*plotScales[1])
            histos["byevt_nroads1_SSclean_%03d"               % (ievt)].Fill(1,(nroads5_SSclean                                          )*plotScales[1])
            histos["byevt_nroads1_SSclean_%03d"               % (ievt)].Fill(2,(                             nroads6_SSclean             )*plotScales[1])
            histos["byevt_nroads2_SSclean_matched_chi2_%03d"  % (ievt)].Fill(0,(nroads5_SSclean_matched_chi2+nroads6_SSclean_matched_chi2)*plotScales[2])
            histos["byevt_nroads2_SSclean_matched_chi2_%03d"  % (ievt)].Fill(1,(nroads5_SSclean_matched_chi2                             )*plotScales[2])
            histos["byevt_nroads2_SSclean_matched_chi2_%03d"  % (ievt)].Fill(2,(                             nroads6_SSclean_matched_chi2)*plotScales[2])
...
#             print ""
        ...
        histos["nroads_5oo6_SSclean_per_event"  ].Fill(nroads5_SSclean)
        histos["nroads_6oo6_SSclean_per_event"  ].Fill(nroads6_SSclean)
        histos["nroads_SSclean_per_event"       ].Fill(nroads5_SSclean+nroads6_SSclean)
        histos["ncombinations_SSclean_per_event"].Fill(ncombs_SSclean)
...
#         print h.GetName(), in_quantiles, quantiles
    ...
    for evtPrint in evtHasAParticle: 
        csortedRoads = ROOT.TCanvas("combs_per_road","combs_per_road", 0, 0, 900,900)
        for ihn, hn in enumerate(range(6)):
            for hname, h in histos.iteritems():
                if (hname.find(("byevt_sortedRoads%d" % (ihn)))==0 and int(h.GetName().split("_")[-1])==evtPrint):
                    if ihn==0:
                        ymax = h.GetMaximum() * 1.2
        ...
        CMS_label()
        csortedRoads.SaveAs("%s/combs_per_road_%03d.png" % (options.outdir,evtPrint))
...
# 
#         elif (h.ClassName() == "TH1F" and h.GetName().find("byevt")>=0):
#             ievt = int(h.GetName().split("_")[-1])
#             if ievt not in evtHasAParticle: continue
#             ymax = h.GetMaximum() * 1.2
#             h.SetMinimum(0); h.SetMaximum(ymax)
# 
#             
...
def drawer_sitrep(histos, options):
    lines=[] ;line = ""
    lines.append(line); line = "--- SITREP ---------------------------------------------------------"
    lines.append(line); line = "--- Using tt{0}, pu{1}, ss={2}, npatterns={3}, ncommomSSclean={4}".format(options.tower, options.pu, options.ss, options.npatterns, options.ncommonSS)
    lines.append(line); line = "--- Variable, mean, 95%% CI, 99%% CI:"
    h = histos["nroads_per_event"]
    lines.append(line); line = "nroads per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ncombinations_per_event"]
    lines.append(line); line = "ncombs per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ntracks_per_event"]
    lines.append(line); line = "ntrks  per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ngoods_per_event"]
    lines.append(line); line = "ngoods per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["nduplicates_per_event"]
    lines.append(line); line = "ndupls per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["nfakes_per_event"]
    lines.append(line); line = "nfakes per event        \t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)

    h = histos["nroads_SSclean_per_event"]
    lines.append(line); line = "nroads SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ncombinations_SSclean_per_event"]
    lines.append(line); line = "ncombs SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ntracks_SSclean_per_event"]
    lines.append(line); line = "ntrks  SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["ngoods_SSclean_per_event"]
    lines.append(line); line = "ngoods SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["nduplicates_SSclean_per_event"]
    lines.append(line); line = "ndupls SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    h = histos["nfakes_SSclean_per_event"]
    lines.append(line); line = "nfakes SSclean per event\t{0:6.4g}\t{1:6.4g}\t{2:6.4g}".format(*h.stats)
    
    prefix = "efficiency_"
    h = histos[prefix + "pt"]
    if (h.denomEntries>0):
        lines.append(line); line = "eff        = {0}/{1} = {2:.3f}".format(h.numEntries, h.denomEntries, float(h.numEntries)/h.denomEntries)
        prefix = "efficiency_SSclean_"
        h = histos[prefix + "pt"]
        lines.append(line); line = "eff_SSclean= {0}/{1} = {2:.3f}".format(h.numEntries, h.denomEntries, float(h.numEntries)/h.denomEntries)
        lines.append(line)

    out = options.outdir.strip("/")
    fileout = out+"/"+out+".txt"
    f = open(fileout, 'w')
    for line in lines:
        f.write(line+"\n")
...
def main():
    ...
    parser.add_argument("--ncommonSS", type=int, default=4, help="min # of layers with same SS needed for road cleaning (default: %(default)s)")
    ...
    parser.add_argument("--nskip", type=int, default=0, help="number of entries to skip (default: %(default)s)")
