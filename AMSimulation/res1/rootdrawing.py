...
import ROOT
from ROOT import TH1, TH1F, TH2F, TProfile, TProfile2D, TFile, TChain, TCanvas, TPad, TLegend, TLatex, TLine, TPolyLine, TBox, TGraph, TGraphErrors, TGraphAsymmErrors, gROOT, gInterpreter, gStyle, gSystem, gPad
...
def struct(*args):
    p = Param(name=args[0],
        ...
        xlow=args[5],
        xup=args[6],
        tlinecolor=args[7], markercolor=args[7],
        fillcolor=args[8]
        )
    return p
...
def struct2D(*args):
    p = Param(name=args[0],
        ...
        ylow=args[9],
        yup=args[10],
        tlinecolor=args[11], markercolor=args[11],
        fillcolor=args[12]
        )
    return p
