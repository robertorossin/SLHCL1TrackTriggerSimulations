...
nbins_invPt = 50
nbins_phi   = 200
a_invPt = TAxis(nbins_invPt,min_invPt,max_invPt)
a_phi   = TAxis(nbins_phi  ,min_phi  ,max_phi  )
...
def getRoadBin(road_invPt, road_phi):
    i_invPt = a_invPt.FindBin(road_invPt)
    i_phi   = a_phi  .FindBin(road_phi  )
    return i_invPt + nbins_invPt*i_phi
...
#                 continue
        ...
        for iroad, (patternRef, patternInvPt, superstripIdsA, stubRefsA) in enumerate(
            ...
            road_invPt = treepatternA.invPt_mean
            road_phi   = treepatternA.phi_mean
            roadList.append((patternRef,6-len(misslayers),misslayers,superstripIdsA,stubRefsA, abs(road_invPt),road_phi,road_trackList,getRoadBin(road_invPt,road_phi)))
...
#         roadListSorted = sorted(roadList, key = operator.itemgetter(0)              )
        ...
        roadListClean = []
#         roadListClean_invPtPhi = []
        roadListClean_invPtPhi_indexMap = {}
#         maxsiblings = 0
        
        nroads5_SSclean, nroads6_SSclean, ncombs_SSclean = 0, 0, 0
        for iroad, road in enumerate(roadListSorted):
            
            if road[8] not in roadListClean_invPtPhi_indexMap:
                roadListClean_invPtPhi_indexMap[road[8]] = 1
...
#         tracks = {}
        ...
        for itrack in xrange(ntracks_all):
            ...
            if chi2Red < options.maxChi2: ntracks += 1
            else:                         continue
...
#         h.SetStats(0)
    ...
    for hname, h in histos.iteritems():
        ...
        if h.ClassName() == "TEfficiency":
            ...
            if (h.GetName().find("_per_event")>=0):
                print "Adding quantiles to histo ", h.GetName()
                displayQuantiles(h,[0.5,0.95,0.99])
