...
void roadMerger::Loop(unsigned int iEv,unsigned merging,TString pName, bool makePlots, bool debug, TString fName)
{
   ...
   if (pName.Contains("sf1L0x2L5x2_nz4")) {
	   ...
	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2L5x2_nz4_pt3_100c_300M.root","READ");
   }
...
//       if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2L5x2_nz6_pt3_300M.root" ,"READ");
	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2L5x2_nz6_pt3_100c_300M.root" ,"READ");
   }
...
//	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2L5x2_nz8_pt3_300M.root" ,"READ");
	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2L5x2_nz8_pt3_100c_300M.root" ,"READ");
   }
...
//       if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2_nz6_pt3_300M.root" ,"READ");
       if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2_nz6_pt3_100c_300M.root" ,"READ");
   }
...
//	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2_nz8_pt3_300M.root" ,"READ");
	   if (merging==8) fmergedPatt = new TFile("/data/rossin/EOS/patternMerging/m8_patternBank_tt27_sf1L0x2_nz8_pt3_100c_300M.root" ,"READ");
   }
...
//	   if (iMerged%100==0) cout << iMerged << "\t" << indFromMerged->size() << endl;
   ...
   std::sort(vmergedPatterns.begin(), vmergedPatterns.end());
   for (unsigned iMerged=0; iMerged<vmergedPatterns.size();++iMerged) {
  	 ...
  	 for (unsigned ip=0; ip<patternIds.size(); ++ip) {
  		 ...
  		 if (ret_patternIdset.second==false) {
  			 std::cout << "ERROR. DUPLICATED PATTERN IN EVENT" << std::endl;
  			 return;
  		 }
  	 }
//  	 if (iMerged%10000==0) std::cout << iMerged << "\t" << mp.getfrequency() << std::endl;
  	 if (totalBankCoverage*partialMergedFreq/totalFreq>targetCoverage) {
  		 mergedbankSizeAtTargetCoverage = iMerged;
  		 std::cout << "Coverage " << totalBankCoverage*partialMergedFreq/totalFreq << " reached with " << mergedbankSizeAtTargetCoverage<< " merged patterns" << std::endl;
